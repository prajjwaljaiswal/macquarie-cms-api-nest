...
import { DailySet50AecsEntity, DailySet50AecsImageEntity } from './dailyset50aecs.entitiy';

@Injectable()
export class DailySet50AecsService {
    constructor(
        @InjectRepository(DailySet50AecsEntity)
        private readonly DailySet50AecsEntity: Repository<DailySet50AecsEntity>,
   ...
   async getTopPicks(){
        return await this.DailySet50AecsEntity
        .createQueryBuilder()
        .select([
           'id',
           'daily_highlight_status',
           'publish_date',
           'en_title',
           'thai_title',
           'en_short_content',
    ...
    async updateDailySP500(data: DailySet50AecsDto){
        const { id } = data;
        return await this.DailySet50AecsEntity
        .createQueryBuilder()
        .update(DailySet50AecsEntity)
        .set({
            ...data
        })
        .where("id = :id", { id })
    ...
    async insertDailySP500(data: createDailySet50AecsDto){
    ...
    async getImage(id){
        const adbanner = await this.DailySet50AecsEntity.findOne({
            select: ['image'],
            where: { id: id }
          });
          if (adbanner?.image) {
    ...
    async updateImage(data: DailySet50AecsImageDto){
        const { id, image } = data;
    ...
    async deleteDailySP500(id: number) {
        return await this.DailySet50AecsEntity.createQueryBuilder()
          .delete()
          .where('id = :id', { id: id })
          .execute();
      }
